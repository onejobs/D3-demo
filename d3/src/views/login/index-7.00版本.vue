...
export default {
  ...
  mounted() {
    ...
    // function drawChart(options, data) {
    ...
    for (var i = 0; i < data.links.length; i++) {
      var jsonObj = {};
      jsonObj.source = data.links[i].source;
      jsonObj.target = data.links[i].target;
      jsonObj.relation = data.links[i].relation;
      jsonObj.sourceImg = data.links[i].sourceImg;
      jsonObj.targetImg = data.links[i].targetImg;
      jsonObj.sourceColor = data.links[i].sourceColor;
      jsonObj.targetColor = data.links[i].targetColor;
      jsonObj.sourceRadius = data.links[i].sourceRadius;
      jsonObj.targetRadius = data.links[i].targetRadius;
      // 根据关系类型添加连接线的颜色
      // jsonObj.lineColor = data.links[i].lineColor;
      switch (data.links[i].relation) {
        case "投资":
      ...
      jsonObj.data = data.links[i].data;
    ...
    let force = d3
      ...
      .velocityDecay(0.5);
    //  .id((d) => d.id) .strength(0.6).distance(Math.floor(height / 4))
    // .strength(-500).distanceMin(50).distanceMax(400)
    //  d3.layout
    //   .force()
    // let force = d3.layout
    // .force() // 生成力导向图的布局函数
    ...
    // .start();
    console.log(force, "mmmmmmmm");
    // 全图缩放器
    // var zoom = d3.behavior.zoom().scaleExtent([0.25, 2]); // 设置最大缩放比例
